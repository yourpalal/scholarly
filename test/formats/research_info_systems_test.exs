defmodule ResearchInfoSystemsTest do
  use ExUnit.Case
  doctest Scholarly.ResearchInfoSystems
  ...
  describe "authors" do
    ...
    test "can load authors with A[1234] and AU" do
      {:ok, [r1]} =
        ["TY  - JOUR", "A3  - 3", "A1  - 1", "AU  - 5", "AU  - 6", "A2  - 2", "A4  - 4",
         "ER  - "]
         |> Enum.join("\r\n")
         |> Scholarly.ResearchInfoSystems.parse()

      assert %Scholarly.Record{authors: ~w(1 2 3 4 5 6)} = r1
    end
  end
end
